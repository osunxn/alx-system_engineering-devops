...
Python script to interact with a REST API, retrieving and processing employee TODO list progress.
...
- [Friends don’t let friends program in shell script](https://www.turnkeylinux.org/blog/shell-scripting-humor)
- [What is an API? In English, please](https://www.youtube.com/watch?v=GZvSYJDk-us)
...
- Understand what the CSV format is
- Understand what the JSON format is
- Apply Pythonic package, module, class, variable, function, and constant name styles
...
- Code must use the `pycodestyle` (version 2.8.*) style
- All files must be executable
- File length will be tested using `wc`
- All modules must have documentation (use `python3 -c 'print(__import__("my_module").__doc__)'`)
- `get` must be used to access dictionary value by key (it won’t throw an exception if the key doesn’t exist in the dictionary)
